...
class AppInfo:
    ...
    def GET(self):
        ...
        return ws_io.ws_output(dict(rt=rt.eid,appinfo=appinfo,updatetime=updatetime),error_info)
    #}}}
...
class AddNativeApp:
    ...
    def POST(self):
        ...
        try:
            ...
            dest_file=os.path.join( config.get('app_download_dir'),'apps',appfile)   #直接复制到远程文件夹中
            #shutil.copy2(ap, dest_file)
            #远程传输到数据服务器 +++20150317
            back_host=config.get('redis_host')   #都存在数据库服务器，同Mongo地址
            rt_c=commands.getstatusoutput( 'scp %s %s:%s'%(ap,back_host,dest_file))
            logging.error('add: scp %s %s:%s!%s',ap,back_host,dest_file,rt_c)
...
class UpdateNativeApp:
    ...
    def POST(self):
        ...
        try:
            ...
            back_host=config.get('redis_host')   #都存在数据库服务器，同Mongo地址
            rt_c=commands.getstatusoutput( 'scp %s %s:%s'%(ap,back_host,dest_file))
            logging.error('update:scp %s %s:%s!%s',ap,back_host,dest_file,rt_c)
            commands.getstatusoutput( 'rm -f %s'%(ap))   #删除临时文件
            if rt_c[0]!=0:
                raise ecode.DB_OP
        ...
        try:
            ...
            if apps_sent:
                for app_sent in apps_sent:
                    ...
                    for temp in temps:   #就一个
                        if app_id==temp[0]:
                            app={'app_id':app_id,'versionCode':versionCode,'url':'/update/secphone.apk'}
                        else:
                            app = app_db.getapp(app_id,versionCode)
                        apps.append(app)
