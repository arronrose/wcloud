...
def add_liveness_to_online_days(online_days,add_days):
    """
    根据用户在线天数对用户的活跃值进行恢复
        ...
        for item in result:
            ...
            if devs!=[]:
                ...
                if(dev_online_info!=None):
                    online = dev_online_info['online']
                    outline = dev_online_info['outline']
                    if int(online)>online_days:        #条件一：在线天数大于要求的天数
                        if int(outline)>add_days:      #条件二：添加的天数不能超过离线的天数
                            new_online = int(online)+add_days
                            new_outline = int(outline)-add_days
                            dev.update({"dev_id":dev_id},{"$set":{"online":new_online,"outline":new_outline}})
                            count+=1
...
def add_liveness_to_use_days(use_days,add_days):
    """
    根据用户在线天数对用户的活跃值进行恢复
    :param online_days:用户的在线天数
    :param add_days:添加的活跃值
        ...
        for item in result:
            ...
            if devs!=[]:
                ...
                if(dev_online_info!=None):
                    online = dev_online_info['online']
                    outline = dev_online_info['outline']
                    if int(online)+int(outline)>use_days and int(online)<20:        #条件一：在线天数大于要求的天数
                        if int(outline)>add_days :      #条件二：添加的天数不能超过离线的天数
                            new_online = int(online)+add_days
                            new_outline = int(outline)-add_days
                            dev.update({"dev_id":dev_id},{"$set":{"online":new_online,"outline":new_outline}})
