...
def is_has_app( app_id ):
    try:
        ...
        logging.error('is_has_app failed. app_id:%s', app_id)
        raise ecode.DB_OP
    return False
...
def add_app( app_id, app_name, apptype, version, versionCode, url, remark ):
    ...
    try:
        ...
        logging.error('add new app failed. app_id:%s', app_id)
    raise ecode.DB_OP
...
def del_app( app_id):
    try:
        ...
        logging.error('del_app failed. app_id:%s', app_id)
    raise ecode.DB_OP
...
def update( app_id, app_name, apptype, version, url, remark):
    try:
        ...
        logging.error('update failed. app_id:%s', app_id)
    raise ecode.DB_OP
...
def get( app_id):
    ...
    try:
        ...
        logging.error('get failed. app_id:%s', app_id)
        raise ecode.DB_OP
    if not r:
...
def get_app_list( key='', sortkey='utime', asc=1, start=0, count=100):
    try:
        ...
        logging.error('get app list failed.')
    raise ecode.DB_OP
