...
class UserLDAP:
    ...
    def get_all_onelevel(self, oudn):
        ...
        try:
            ...
            logging.error('ldap onelevel' )
            raise ecode.READ_LDAP_FAIL
        finally:
            del self

    def get_all_onelevel_ou(self, oudn):
        """get all ous on one level for the based oudn 查找oudn下的所有群组节点"""
        try:
            res_id = self.lobj.search( oudn, ldap.SCOPE_ONELEVEL
    ...
    def get_all(self):
        ...
        try:
            ...
            logging.error('ldap all' )
        raise ecode.READ_LDAP_FAIL

    def get_ou_tree(self,base_dn):
        """获取LDAP服务器上的树架构"""
        try:
            ous = []
            res_id = self.lobj.search( base_dn, ldap.SCOPE_ONELEVEL
                    , 'objectClass=%s'%("organizationalUnit"), None)
            while self.lobj:
                rtype,rdata = self.lobj.result(res_id,0)
                if not rdata:
                ...
                ou["ous"]=self.get_ou_tree(rdata[0][0])
                ous.append(ou)
            return ous
    ...
    def get_user(self, userid):
        ...
        try:
            ...
            logging.error('get_user, search user,user=%s', userid )
            raise ecode.READ_LDAP_FAIL
...
#             raise ecode.USER_NOT_EXIST  
    ...
    def oudn_has_username(self,username,oudn): #精确查找，返回值为list，使用search()
        try:
            res_id =self.lobj.search(oudn, ldap.SCOPE_ONELEVEL, "cn=%s"%(username,), None)
            rtype,rdata = self.lobj.result(res_id, 0)
            print(rdata)
            if rdata==[]:
                return False
            ...
            logging.error('oudn_has_username failed, username=%s,oudn=%s', username,oudn)
            raise ecode.READ_LDAP_FAIL
    ...
    def get_uid_by_username(self, username):
        ...
        try:
            ...
            logging.error('get_uid_by_username, search user,user=%s', username )
            raise ecode.READ_LDAP_FAIL
    ...
    def is_has_pnumber(self,pnumber):
        ...
        try:
            ...
            logging.error('is_has_pnumber failed ,pnumber=%s', pnumber )
            raise ecode.READ_LDAP_FAIL
...
#             raise ecode.USER_NOT_EXIST  
    ...
    def check_pw(self, userid, pw):
        ...
        try:
            ...
            logging.error('check_pw, simple_bind_s' )
        return False
...
#             pass
    ...
    def modify_user(self,target_userdn,old,new):
        try:
            # Convert place-holders for modify-operation using modlist-module
            ldif = modlist.modifyModlist(old,new)

            # Do the actual modification
            self.lobj.modify_s(target_userdn,ldif)
