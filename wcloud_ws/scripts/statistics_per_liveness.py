...
def data_statistics():
    ...
    for item in result:
        if item['devs']!=[]:
            ...
            sum_liveness += float(dev_onlinestate['online'])/(float(dev_onlinestate['online'])+float(dev_onlinestate['outline']))
        else:
            continue
    average_online_days = str(float(sum_online_days)/active_count)
    average_use_days = str(float(sum_online_days+sum_out_line_days)/active_count)
    print "激活设备数量:"+str(active_count)
    print "总活跃天数:"+str(sum_online_days)
    print "平均活跃天数:"+str(float(sum_online_days)/active_count)
    print "总使用天数:"+str(sum_online_days+sum_out_line_days)
    print "平均使用天数:"+str(float(sum_online_days+sum_out_line_days)/active_count)
    print "平均活跃度1:"+str(float(sum_online_days)/float(sum_online_days+sum_out_line_days))
    print "平均活跃度2:"+str(sum_liveness/float(active_count))
...
平均活跃天数:5.5263957463
总使用天数:160410
平均使用天数:60.9229016331
平均活跃度1:0.0907113022879
平均活跃度2:0.29617400918
"""
