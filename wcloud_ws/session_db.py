...
class Session:
    ...
    def create(self, uid, dev_id, ip ):
        ...
        try:
            ...
            if oldsid:
                #+++20150313   删除多余的websocket，保持一人一通道
                config.delete_subscreber(oldsid)
            ...
            sid = ''
            logging.error('create sid failed. uid:%s,dev_id:%s,ip:%s'
                    , uid, dev_id, ip)
    ...
    def del_by_user(self, uid, dev_id):
        ...
        try:
            ...
            logging.error('del_by_user failed. uid:%s,dev_id:%s'
                    , uid, dev_id)
    ...
    def del_by_sid(self, sid, ip):
        ...
        try:
            ...
            #注销websocket通道+++20150313
            config.delete_subscreber(sid)
            if uid and dev_id:
                self.rdb.delete(s)
                self.rdb.hdel(uid,dev_id)
        except Exception,ex:
            logging.error('del_by_sid failed. sid:%s,ip:%s'
                    , sid, ip)
    ...
    def get_sid(self, uid, dev_id ):
        ...
        try:
            ...
            sid = ''
            logging.error('get_sid failed. uid:%s,dev_id:%s'
                    , uid, dev_id)
        return sid
    ...
    def get_user(self, sid, ip ):
        ...
        try:
            ...
            u = ''
            logging.error('get_user failed. sid:%s,ip:%s'
                    , sid, ip)
        return u
